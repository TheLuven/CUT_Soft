...
public class Login{
    ...
    public Login(DatabaseManager databaseManager, Stage primaryStage){
        ...
        //this.window.getTopPanel().getChildren().remove(this.window.getDisconnectButton());  // Supprime le bouton de déconnexion
        //this.window.getTopPanel().getChildren().remove(this.window.getProfileButton());     // Supprime le bouton de profil
    ...
    // 6 première lettre nom et 2 première lettre prénom
    public void id_check() {
        ...
        if (id_get == -1) {
            ...
            if (result) { // Si le mot de passe est correct
                ...
                Gestion gestion = new Gestion(dbManager.getAllTeacherClassMapByTeacher(dbManager.getTeacherByID(id_get)),screenBounds,primaryStage,dbManager);
